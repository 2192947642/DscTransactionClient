...
public class DcsRequestInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if(!isBegin){//如果当前是分布式事务调用
            httpServletRequest.setAttribute(DCSAttributes.isDscTransaction,true);//设置当前是分布式事务的请求
    ...
    public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) throws Exception {
        if(!request.getAttribute(DCSAttributes.isDscTransaction).equals(true)) return;
